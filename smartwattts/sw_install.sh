...
case $VAR in
    ...
    3) echo "Getting Smartwatts...";
       curl -s https://api.github.com/repos/powerapi-ng/smartwatts-formula/releases/latest |  grep "browser_download_url.*\
deb" | cut -d : -f 2,3 | tr -d \" | wget -qi -;
...
case $VAR in
    ...
    2) echo "Getting HWPC Sensor...";
       curl -s https://api.github.com/repos/powerapi-ng/hwpc-sensor/releases/latest |  grep "browser_download_url.*deb" | \
cut -d : -f 2,3 | tr -d \" | wget -qi -;
       echo "HWPC Sensor downloaded";
       echo "Building...";
       sudo apt install ./hwpc-sensor-*.deb || rm hwpc-sensor-*.deb;;
    3)curl -s https://api.github.com/repos/powerapi-ng/hwpc-sensor/releases/latest  |  grep "browser_download_url*" | grep\
 --invert-match ".deb" |  cut -d : -f 2,3 | tr -d \" | wget -qi -;;
    *) echo "Not a valid choice. Installation aborted"; exit 0;;
esac
